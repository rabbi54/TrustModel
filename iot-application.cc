...
namespace ns3{
        ...
        static TypeId tid =
            TypeId ("ns3::IoTApplication")
                ...
                .AddAttribute ("Interval", "Broadcast Interval", TimeValue (MilliSeconds (100)),
                                MakeTimeAccessor (&IoTApplication::m_broadcast_time),
                                MakeTimeChecker ());
        return tid;
    }

    TypeId
    IoTApplication::GetInstanceTypeId () const
    {
        return IoTApplication::GetTypeId ();
    }
    ...
    void
    IoTApplication::Setup(Ptr<Socket> _speak_socket, Ptr<Socket> _list_socket, uint32_t pktSize, uint32_t nPkts, DataRate dataRate, uint16_t _port)
    {
    ...
    void
    IoTApplication::StartApplication ()
    {
        ...
        listener_socket->SetRecvCallback(MakeCallback( &IoTApplication::ReceivePacket, this));

        listener_socket->SetAcceptCallback (
        MakeNullCallback<bool, Ptr<Socket>, const Address &> (),
        MakeCallback (&IoTApplication::HandleAccept, this));
    ...
    //We will periodically (every 1 second) check the list of neighbors, and remove old ones (older than 5 seconds)
        ...
        if(find(sp_indexs.begin(), sp_indexs.end(), selfAddrIndex) == sp_indexs.end())
        {
            ...
            Simulator::Schedule (Seconds(service_taking_period) + random_offset, &IoTApplication::ScheduleService, this);
            Simulator::Schedule (Seconds (trust_request_period) + random_offset, &IoTApplication::RequestForTrust, this);
        }
    }
    ...
    void 
    IoTApplication::StopApplication (void)
    {
        m_running = false;
    ...
    void 
    IoTApplication::HandleAccept (Ptr<Socket> s, const Address& from)
    {
        s->SetRecvCallback (MakeCallback (&IoTApplication::ReceivePacket, this));
        m_socketList.push_back (s);
    }


    void
    IoTApplication::AddSPIndex(int i)
    {
        sp_indexs.push_back(i);
    }
    void
    IoTApplication::AddTTPAddr(Address a){
        ttp.push_back(a);
    }

    void 
    IoTApplication::AddSPAddr(Address a)
    {
        sp.push_back(a);
    }

    void 
    IoTApplication::AddIoTAddr(Address a)
    {
        iot.push_back(a);
    }
    void 
    IoTApplication::AddTTPPos(Vector a){
        ttp_pos.push_back(a);
    }

    void
    IoTApplication::SetPos(Vector a){
        pos = a;
    }
    
    void 
    IoTApplication::SetSelfAddrIndex(uint16_t i)
    {
        selfAddrIndex = i;
    }

    void
    IoTApplication::RequestForTrust()
    {
        ...
        Time random_offset = MilliSeconds (rand2->GetValue (50, 3000));

        Simulator::Schedule ( Seconds(trust_request_period) + random_offset,
        &IoTApplication::RequestForTrust, this);


        Ptr<UniformRandomVariable> rand = CreateObject<UniformRandomVariable> ();
        Time offset = MilliSeconds (rand->GetValue (100, 700));

        Simulator::Schedule ( Seconds(3.0) + offset,
        &IoTApplication::ScheduleRequestForTrust, this);
    }

    void
    IoTApplication::ScheduleRequestForTrust()
    {
    ...
    void
    IoTApplication::ScheduleReply(Time ts, uint16_t my_adrs, Address to_adrs)
    {
    ...
    void
    IoTApplication::ScheduleService()
    {
        ...
        if(window_cell == 2 || window_cell == 3)
        {
            ...
            // for(auto x : sp_indexs)
            //     cout<<x<<'\t';
            // cout<<"\n";
            // cout<<"Taking service from a sp "<<s<<"  IP is   "<<InetSocketAddress::ConvertFrom(iot[s]).GetIpv4()<<endl;
        }

        // if(window_cell == 13 || total_service_taken % (int)(trust_request_period/service_taking_period) == 23 || total_service_taken % (int)(trust_request_period/service_taking_period) == 11)
        // {
        //     s = honest_iot;
        // }
        ...
        Ptr<UniformRandomVariable> rand = CreateObject<UniformRandomVariable> ();
            Time random_offset = MilliSeconds (rand->GetValue (5, 500));

        Simulator::Schedule ( Seconds(service_taking_period) + random_offset,
            &IoTApplication::ScheduleService, this);
    }


    void 
    IoTApplication::CallForService(Address a)
    {
    ...
    void
    IoTApplication::ReceivePacket (Ptr<Socket> sock)
    {
        Address from;
        Ptr<Packet> packet;
        while( (packet = sock->RecvFrom (from)) )
        {
            ...
            if(type == PacketData::RESPONSE_FOR_SERVICE)
            {
                ...
                // cout<<"Received Service from \t"<< pd.GetFromAddress()<<"\tafter "<<(send_time).GetMilliSeconds()<<endl;
            ...
            if(type == PacketData::REQUEST_FOR_SERVICE)
            {
                ...
                Ptr<UniformRandomVariable> rand = CreateObject<UniformRandomVariable> ();
                Time service_rtime = MilliSeconds (rand->GetValue (1, max_service_time_delay));
                Simulator::Schedule(service_rtime, &IoTApplication::ScheduleReply, this, service_rtime, addrss_ind, adrs);
                
                // cout<<"Service from "<<InetSocketAddress::ConvertFrom(iot[selfAddrIndex]).GetIpv4()<<" at delay is "<<service_rtime.GetMilliSeconds()<<endl;
    ...
    bool
    IoTApplication::IsSuccessFulTrx(Time rcvT, Time sndT)
    {
    ...
    void 
    IoTApplication::UpdateTrustWindow(Address a, bool isSuccessFul)
    {
